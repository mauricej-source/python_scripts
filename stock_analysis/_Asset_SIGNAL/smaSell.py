...
def check_sell_signal(df):
    """
    Checks for a sell signal on the latest available data point.
    
    Returns:
        bool: True if a sell signal is found, False otherwise.
    """
    # Ensure there are at least two data points to check for a crossover
    if len(df) < 2:
        return False
    ...
    # Sell Signal: Price crosses SMA_Fast to the downside
    sell_signal_current = latest_data['close'] < latest_data['SMA_Fast']
    sell_signal_prev = prev_data['close'] >= prev_data['SMA_Fast']
    
    return sell_signal_current and sell_signal_prev
def get_last_sell_signal_date(df):
    """
    Finds the date of the most recent sell signal in the DataFrame.
    
    Returns:
        datetime.date: The date of the last sell signal, or None if no signal is found.
    """
    # Create a boolean series for all sell signals
    sell_signals = (df['close'] < df['SMA_Fast']) & (df['close'].shift(1) >= df['SMA_Fast'].shift(1))
    
    # Get the index (date) of the last True value
    last_signal_date = None
    if sell_signals.any():
        last_signal_date = sell_signals[sell_signals].index[-1].date()
        
    return last_signal_date

# --- Main Execution ---
if __name__ == "__main__":
    print("--- Analyze Stock for SMA Sell Signal ---")
    ...
    print(f">> ")
    print(f">> --------------------------------------------------------------------")
    print(f">> BEGIN Processing - {ticker_symbol} - Determining SMA Sell Signal ...")
    ...
    if downloaded_csv_path:
        ...
        if not data.empty:
            ...
            # Check for sell signal and print the result
            has_sell_signal = check_sell_signal(data)
            print(f">>    Ticker Symbol - {ticker_symbol} - SMA Sell => {has_sell_signal}")
            
            # Find the date of the last sell signal and print it
            last_sell_date = get_last_sell_signal_date(data)
            print(f">>    Ticker Symbol - {ticker_symbol} - Last SMA Sell => {last_sell_date}")            
        else:
            print(f">>    Failed to load data from {downloaded_csv_path}.")
            
    print(f">> END Processing - {ticker_symbol} - Determining SMA Sell Signal ...")
    print(f">> --------------------------------------------------------------------")
    print(f">> ")            
